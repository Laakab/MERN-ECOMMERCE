...
import { Container, Row, Col, Form, Button, Card, Spinner, InputGroup, Alert, Image } from 'react-bootstrap';
import { FaUserPlus, FaLock, FaEnvelope, FaUser, FaPhone, FaMapMarkerAlt, FaCheck, FaTimes, FaEye, FaEyeSlash, FaMoneyBill } from 'react-icons/fa';
...
const Signup = () => {
  ...
  // Add payment amount based on role
  useEffect(() => {
    if (formData.role === 'shop') {
      setFormData(prev => ({ ...prev, paymentAmount: '500' })); // $500 for shop
    } else if (formData.role === 'deliver') {
      setFormData(prev => ({ ...prev, paymentAmount: '300' })); // $300 for delivery
    } else {
      setFormData(prev => ({ ...prev, paymentAmount: '', transactionId: '', paymentScreenshot: null }));
    }
  }, [formData.role]);
  ...
  const validateForm = () => {
    ...
    // Payment validation for shop and deliver roles
    if ((formData.role === 'shop' || formData.role === 'deliver') && !formData.transactionId) {
      newErrors.transactionId = 'Transaction ID is required';
    }

    if ((formData.role === 'shop' || formData.role === 'deliver') && !formData.paymentScreenshot) {
                    ...
                    <Form onSubmit={handleSubmit}>
                      ...
                      <Form.Group className="mb-4">
                        ...
                        >
                          ...
                          >
                          <option value="deliver">Select Registration</option>
                            <option value="deliver">Delivery Partner</option>
                      ...
                      {/* Payment Information for Shop and Deliver */}
                      {(formData.role === 'shop' || formData.role === 'deliver') && (
                        <motion.div
                          initial={{ opacity: 0, height: 0 }}
                          animate={{ opacity: 1, height: 'auto' }}
                          transition={{ duration: 0.3 }}
                          className="payment-section mb-4 p-3 bg-light rounded"
                        >
                          <h5 className="mb-3 text-center">Payment Information</h5>
                          <Alert variant="info" className="mb-3">
                            <strong>Payment Required:</strong> Please pay ${formData.paymentAmount} to register as a {formData.role} on my Number  <strong>03064155248</strong> After Enter Your Transaction ID <strong>And</strong> Payment Screenshot <strong>OtherWise </strong>will not Accept Your Account.
                          </Alert>
